...
rm -rf .git .github
rm CODE_OF_CONDUCT.md LICENSE README.md COPYRIGHT version.txt
...
sed -i '' -e 's|org.gradle.configuration-cache=true|org.gradle.configuration-cache=false # disabled for app-services|' gradle.properties
git commit gradle.properties -m "Disable gradle cache"
