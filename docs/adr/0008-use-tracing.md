...
Changing this max level to, say, `Trace` can cause performance regressions in other crates - eg, see [this desktop bug](https://bugzilla.mozilla.org/show_bug.cgi?id=1874215) for one example.
...
* The default way of using the tracing crate continues to support the `RUST_LOG` environment variable and writes
  to `stdout`, so most developers who are running tests etc should notice no difference.
...
* We investigate using the same capability for error reporting, both on mobile and desktop.
...
However, there are advantages in avoiding the use of the `log` crate, so we should aim for all our own crates to migrate to `tracing`.

Note that in the first instance, the end result of this proposal does not end up with log output going anywhere
different from where it goes today - for example, Android will continue to see all output in `logcat`, iOS will
see the logs in the same place, desktop will arrange to send the logs to the same place they are today depending on the component etc.
This ADR only changes the mechanism, but not where they go.
...
simply subscribe to the most verbose log level and perform additional filtering after the log event has been dispatched.
